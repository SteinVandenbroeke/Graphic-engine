...
namespace
{
	...
	void print_system(std::ostream&out, LParser::LSystem& system)
	{
	...
	void parse_rules(std::set<char> const& alphabet, std::map<char, std::vector<std::string>>& rules, stream_parser& parser, bool parse2D)
	{
		...
		while (true)
        {
			...
			if (rules.find(c) == rules.end())
                v.clear();//throw LParser::ParserException(std::string("Double entry '") + c + "' in rules specification ", parser.getLine(), parser.getCol());
			...
			int specialCharLocation = 0;
            if(rule.find(']') != std::string::npos)
                specialCharLocation = rule.find(']') + 1;

			if (!isValidRule(alphabet, rule.substr(specialCharLocation, rule.size()), parse2D))
...
std::string LParser::LSystem::get_replacement(char c)
{
	assert(get_alphabet().find(c) != get_alphabet().end());
	if(replacementrules.find(c)->second.size() > 1){
        std::vector<std::string> v = replacementrules.find(c)->second;
        double total = 0;
        double randomNR = ((rand() % 100) + 1);
        for(int i = 0; i < v.size(); i++){
            total += (getChanceFromRule(v[i]) * 100);
            if(randomNR <= total){
                return v[i];
            }
        }
        throw LParser::ParserException(std::string("foute waarden, totaal van alle kansen is niet 100%!"),0, 0);
	}
	return replacementrules.find(c)->second[0];
}
...
const double LParser::LSystem::getChanceFromRule(std::string &rule) const {
    std::string number = "";
    int counter = 1;
    if(rule[0] != '[' || !rule.find(']'))
        throw LParser::ParserException(std::string("foute waarden, geen kans meegeven!"),0, 0);
    while(rule[counter] != ']'){
        number += rule[counter];
        counter++;
    }
    rule = rule.substr(counter + 1,rule.size());
...
std::ostream& LParser::operator<<(std::ostream& out, LParser::LSystem2D & system)
{
...
std::ostream& LParser::operator<<(std::ostream&out, LParser::LSystem3D & system)
{
	print_system(out, system);
	out << "Iterations = " << system.get_nr_iterations() << std::endl;
	return out;
