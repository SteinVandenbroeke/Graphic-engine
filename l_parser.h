	...
	*/
	...
	class LSystem
	{
		protected:
			...
			/**
			 * get the chance number from a rule
			 */
            const double getChanceFromRule(std::string &rule) const;
	 ...
	 */
	std::ostream& operator<<(std::ostream& out, LSystem2D & system);
	 ...
	 * \return		The outputstream the L-System was written to
	 */
	std::ostream& operator<<(std::ostream& out, LSystem3D & system);
